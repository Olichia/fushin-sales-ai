...
with performance_tab:
    ...
    if performance_chart.empty:
        ...
        with st.expander("指標定義說明（策略分類、資料信心）"):
            st.markdown(
                """
                **策略分類（建議延續／持續觀察／建議檢討）**

                依「淨增益／日」與全體活動單位淨增益中位數比較判定：

                - 建議檢討：淨增益／日為負，不論是否達中位數。
                - 建議延續：淨增益／日為非負值，且達全體活動單位
                  中位數以上。
                - 持續觀察：淨增益／日為非負值，但未達中位數。

                **資料信心（較高／較低）**

                以下任一情況成立即視為「較低」，否則為「較高」：

                - 涵蓋天數少於 2 天。
                - 有樣本量不足的提示（瀑布法配對樣本量小）。
                - 有使用代理牌價的提示。

                此判定基礎與「活動成效分析」頁面的「資料信心」
                （較高／中等／較低，依觀察期間是否完整落在資料
                涵蓋範圍內判定）不同，兩者不可直接比較。

                **持續觀察建議**

                策略分類與資料信心會隨資料更新而變動，建議定期
                重新整理分析結果，並優先檢視「建議檢討」且資料
                信心「較低」的活動單位，避免只看單次快照就下定論。
                """
            )
